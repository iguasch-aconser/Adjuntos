...
pageextension 60812 ExtractImport120 extends "Posted Purchase Receipts"
{
    actions
    {
        addfirst(processing)
        {
            action(DownloadAttachments112)
            {
                ApplicationArea = All;
                ...
                trigger OnAction()
                var
                    MyRecord: Record "Purch. Rcpt. Header";
                ...
                begin
                    ...
                    if MyRecord.FindSet(false) then begin
                        ZipFileName := Format(Database::"Purch. Rcpt. Header") + '.zip';
                            ...
                            if DocumentAttachment.FindSet(false) then
                                repeat
                                    if DocumentAttachment."Document Reference ID".HasValue then begin
                                        ...
                                        FullFileName := Format(Database::"Purch. Rcpt. Header") + '-' + Format(MyRecord."No.") + '-' + DocumentAttachment."File Name" + '.' + DocumentAttachment."File Extension";
    ...
    begin
        ...
        foreach EntryListKey in EntryList do begin
            ...
            if not (TableID = Database::"Purch. Rcpt. Header") then
